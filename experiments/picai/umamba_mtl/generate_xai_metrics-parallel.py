...
def worker_fn(rank, num_gpus, gpu_ids):
    gpu_id = gpu_ids[rank]
    ...
    for sample_idx, batch in enumerate(tqdm(dl, desc=f"GPU {gpu_id} (rank {rank})", position=rank)):
        ...
        if predicted_positive:
            ...
            saliency_map = attribute_fn.attribute(x, target=0, abs=True)
        ...
        print(f"[GPU {gpu_id}][{sample_idx}] {case_id}: {classification.upper()} | confidence={confidence}% | pz={pca_in_pz} tz={pca_in_tz}")
...
if __name__ == "__main__":
    ...
    if num_gpus == 1:
        ...
        mp.spawn(worker_fn, args=(num_gpus, GPUS), nprocs=num_gpus, join=True)
